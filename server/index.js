...
function startServer(initialPort, attempt = 0) {
  const maxAttempts = 10;
  const basePort = Number(initialPort) || 8080;
  ...
  const handleListenError = (error) => {
    if (error.code === 'EADDRINUSE') {
      ...
      if (nextAttempt >= maxAttempts) {
        console.error(
          `Unable to start server: port ${candidatePort} is busy and retries exhausted.`
        );
        process.exit(1);
      }

      console.warn(
        `Port ${candidatePort} is already in use. Retrying on ${candidatePort + 1}...`
      );
      server.removeListener('error', handleListenError);
      startServer(initialPort, nextAttempt);
      return;
    }
  ...
  server.listen(candidatePort, () => {
    console.log(`Crypto Market API running on port ${candidatePort}`);
  });
}
