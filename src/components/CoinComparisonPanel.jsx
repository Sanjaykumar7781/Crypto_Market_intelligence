...
export function CoinComparisonPanel() {
  ...
  return (
    ...
    >
      {/* Input Section */}
      <motion.div
        className="p-6 rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white backdrop-blur-md"
        whileHover={{ borderColor: 'rgba(255,255,255,0.2)' }}
      >
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Compare Cryptocurrencies</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            >
              ...
              <select
                ...
                className="w-full px-4 py-3 rounded-lg bg-white border border-gray-200 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent capitalize hover:border-gray-300 transition-all"
              >
            ...
            >
              <motion.button
                ...
                className="p-3 rounded-full border border-gray-200 bg-white hover:bg-gray-50 transition-all text-gray-900"
                title="Swap coins"
              >
                ⇄
              </motion.button>
          ...
          <motion.button
            ...
            disabled={loading}
            className="w-full px-6 py-3 rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-gray-900 font-semibold transition-all flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
                <motion.div
...
function QuickMetricsDisplay({ metrics }) {
  ...
  return (
    <motion.div
      ...
      className="p-6 rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white backdrop-blur-md"
    >
      <h3 className="text-xl font-bold text-gray-900 mb-4">📊 Market Metrics</h3>
        ...
        {data.metrics && (
          ...
          >
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-gray-500 text-xs mb-1">Price Ratio</p>
              ...
              <div>
                <p className="text-gray-500 text-xs mb-1">Market Cap Ratio</p>
                <p className="text-gray-900 font-semibold">
                  {data.metrics.marketCapRatio?.toFixed(2)}x
                </p>
              </div>
              <div>
                <p className="text-gray-500 text-xs mb-1">Performance Gap</p>
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900">🏆 AI Verdict</h3>
          <motion.div
            initial={{ scale: 0.5 }}
            animate={{ scale: 1 }}
            className="px-4 py-2 rounded-lg bg-gradient-to-r from-yellow-400 to-orange-500 text-gray-900 font-bold text-lg"
          >
...
function ComparisonRow({ label, valueA, valueB, symbolA, symbolB, highlight = false, isPercentage = false }) {
  return (
    <motion.div
      ...
      className={`p-4 rounded-lg border ${highlight ? 'border-blue-500/30 bg-blue-500/5' : 'border-gray-200 bg-gray-50'}`}
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`font-semibold text-gray-900 ${isPercentage && valueA.includes('-') ? 'text-red-400' : isPercentage ? 'text-green-400' : ''}`}>
              {valueA}
            </p>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`font-semibold text-gray-900 ${isPercentage && valueB.includes('-') ? 'text-red-400' : isPercentage ? 'text-green-400' : ''}`}>
              {valueB}
            </p>
          </div>
        </div>
...
function RiskGrowthCard({ title, coinA, coinB, valueA, valueB, type }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, scale: 1 }}
      className="p-4 rounded-xl border border-gray-200 bg-gray-50 backdrop-blur-md"
    >
      <p className="text-gray-500 text-sm mb-4 font-medium">{title}</p>
