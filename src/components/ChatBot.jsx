  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
      <AnimatePresence>
        {chatOpen && (
          <motion.div
            ...
            className="w-[360px] sm:w-[380px] rounded-3xl border border-gray-200 bg-white/95 shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-gray-200 bg-white p-4">
              ...
              <button
                ...
                className="p-1.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-500 hover:text-gray-900 transition"
              >
                <X size={14} />
              </button>
            </div>
            ...
            >
              {messages.map((message) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-xs font-semibold leading-relaxed shadow-sm border ${
                        isBot
                          ? 'bg-[#F8FAFC] border-[#E5E7EB] text-[#111827] rounded-tl-none'
                          : 'bg-gradient-to-br from-[#2563EB] to-[#7C3AED] border-transparent text-white rounded-tr-none shadow-md'
                      }`}
                    >
                      {message.text}
                    </div>
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="flex gap-1 items-center bg-gray-50 border border-gray-200 rounded-2xl rounded-tl-none px-4 py-3 w-fit">
            ...
            {/* Suggested Prompts */}
            <div className="px-4 py-2 bg-white border-t border-gray-200 flex flex-wrap gap-1.5 items-center">
              ...
              {suggestedPrompts.map((prompt) => (
                <button
                  ...
                  disabled={loading}
                  className="rounded-full bg-white border border-[#E5E7EB] hover:bg-blue-600 hover:border-blue-600 text-[9px] font-bold text-[#111827] hover:text-white px-3 py-1.5 transition-all duration-200 shadow-sm"
                >
                  {prompt}
                </button>
              ))}
            ...
            >
              <div className="flex gap-2">
                <input
                  ...
                  disabled={loading}
                  className="flex-1 rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2 text-xs text-gray-900 outline-none focus:border-cyanGlow/40 transition placeholder:text-gray-500"
                />
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-r from-cyanGlow via-blue-500 to-purple-600 text-gray-900 shadow-[0_4px_20px_rgba(14,165,233,0.35)] hover:shadow-[0_4px_30px_rgba(14,165,233,0.5)] transition duration-200"
      >
