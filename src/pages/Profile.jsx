    ...
    return (
      <PageShell>
        <div className="min-h-[500px] flex items-center justify-center">
          <div className="text-gray-500 text-sm font-bold animate-pulse">Loading profile settings...</div>
        </div>
      </PageShell>
    );
  }
    ...
    <PageShell>
      ...
      >
        ...
        <div>
          ...
          <h1 className="mt-3 text-4xl font-black text-gray-900 bg-gradient-to-r from-gray-900 via-gray-600 to-gray-400 bg-clip-text text-transparent">User Profile settings</h1>
          <p className="text-xs text-gray-500 mt-1">Manage credentials, preferences, and view platform metrics</p>
        </div>
        ...
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
          <div className="metric-card-premium group" style={{ '--glow-start': 'rgba(6,182,212,0.2)', '--glow-end': 'rgba(139,92,246,0.15)' }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-gray-500">Portfolio</span>
            </div>
            <p className="mt-3 text-xl font-black text-gray-900">{currency(48250, false, currencyCode)}</p>
          </div>
          <div className="metric-card-premium group" style={{ '--glow-start': 'rgba(245,158,11,0.2)', '--glow-end': 'rgba(139,92,246,0.15)' }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-gray-500">Watchlist</span>
            </div>
            <p className="mt-3 text-xl font-black text-gray-900">{watchlistItems.length} Saved</p>
          </div>
          <div className="metric-card-premium group" style={{ '--glow-start': 'rgba(139,92,246,0.2)', '--glow-end': 'rgba(59,130,246,0.15)' }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-gray-500">AI usage</span>
            </div>
            <p className="mt-3 text-xl font-black text-gray-900">142 Requests</p>
          </div>
          <div className="metric-card-premium group" style={{ '--glow-start': 'rgba(47,244,166,0.2)', '--glow-end': 'rgba(139,92,246,0.15)' }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-gray-500">Tier Status</span>
            </div>
            <p className="mt-3 text-xl font-black text-gray-900">VIP Premium</p>
          </div>
        </div>

        {/* Profile Card */}
        {profile && (
          <div className="glass-premium rounded-2xl p-6 border border-gray-200 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-48 h-48 bg-cyanGlow/5 blur-3xl pointer-events-none rounded-full" />
            
            {/* Avatar Section */}
            <div className="flex flex-col sm:flex-row items-center gap-6 pb-6 border-b border-gray-200 relative z-10">
              <div className="relative">
                {formData.avatarUrl && !isEditing ? (
                  <img
                    src={formData.avatarUrl}
                    alt={formData.name}
                    className="w-20 h-20 rounded-full object-cover border border-cyanGlow/40 p-1 bg-gray-50 shadow-[0_0_15px_rgba(40,215,255,0.2)]"
                  />
                ) : (
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-cyanGlow via-blue-500 to-purple-600 flex items-center justify-center border border-gray-200 shadow-[0_0_15px_rgba(40,215,255,0.2)]">
                    <User className="w-9 h-9 text-white" strokeWidth={2.4} />
                  </div>
                )}
              </div>
              <div className="text-center sm:text-left">
                <h2 className="text-xl font-black text-gray-900 tracking-tight">{formData.name || 'Anonymous User'}</h2>
            ...
            <div className="mt-6 relative z-10">
              {isEditing ? (
                <div className="space-y-5">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Avatar URL</label>
                    <input
                      type="url"
                      name="avatarUrl"
                ...
                <div className="space-y-6">
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      ...
                      <p className="text-gray-900 font-extrabold text-sm mt-1.5">{formData.name || 'Not configured'}</p>
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      <label className="text-[10px] font-bold uppercase text-gray-500">Email Address</label>
                      <p className="text-gray-900 font-extrabold text-sm mt-1.5 truncate">{profile.email}</p>
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      <label className="text-[10px] font-bold uppercase text-gray-500">Phone</label>
                      <p className="text-gray-900 font-extrabold text-sm mt-1.5">{profile.phone || 'Not configured'}</p>
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      <label className="text-[10px] font-bold uppercase text-gray-500">Age</label>
                      <p className="text-gray-900 font-extrabold text-sm mt-1.5">{profile.age ?? 'Not configured'}</p>
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      <label className="text-[10px] font-bold uppercase text-gray-500">Account Type</label>
                      <p className="text-gray-900 font-extrabold text-sm mt-1.5 capitalize">{profile.provider || 'credentials'}</p>
                    </div>
                  </div>
            ...
            <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5 space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isSaving}
                  className="flex-1 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-black uppercase tracking-wider py-2 text-xs rounded-xl transition"
                >
