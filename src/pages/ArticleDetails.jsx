    ...
    <PageShell>
      <article className="mx-auto max-w-4xl">
        <div className="flex flex-wrap gap-2">
          {(Array.isArray(article.tags) ? article.tags : []).map((tag) => <span key={tag} className="rounded-full bg-gray-50 px-3 py-1 text-xs font-bold text-cyanGlow">{tag}</span>)}
        </div>
        ...
        <div className="mt-8 glass rounded-lg p-6 text-lg leading-8 text-gray-300">
          <p>{article.body}</p>
          <p className="mt-5">
